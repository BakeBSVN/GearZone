...
from django.shortcuts import get_object_or_404, redirect, reverse, get_list_or_404
...
def export_selected_products(request):
    ...
    products = Product.objects.filter(id__in=product_ids).values("name", "description")
    if not products.exists():
        return HttpResponse("No products found", status=404)

    # Fetch products with price and stock separately
    products_with_price = Product.objects.filter(id__in=product_ids)
    variants = ProductVariant.objects.filter(product_id__in=product_ids).prefetch_related('stock_set')

    # Convert MoneyField to float and get stock
    product_data = []
    for product, product_with_price in zip(products, products_with_price):
        ...
        # Get stock from related Stock model
        product_data.append({
            "name": product["name"],
...
#     return TemplateResponse(request, "dashboard/product/category_bulk_update.html", {"form": form})
