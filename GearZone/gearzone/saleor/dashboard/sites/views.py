...
from .forms import CustomKeyboardSlideForm

def manage_slides(request):
    slides = CustomKeyboardSlide.objects.all()
    if request.method == "POST":
        form = CustomKeyboardSlideForm(request.POST, request.FILES)
        if form.is_valid():
            form.save()
            messages.success(request, "Slide added successfully.")
            return redirect("dashboard:manage-slides")
    else:
        form = CustomKeyboardSlideForm()
    return render(request, "dashboard/content/manage_slides.html", {"form": form, "slides": slides})
